  ...
  <div class="col-md-6">
    <div class="card" style="border-top: 4px solid #667eea;">
      <div class="card-header" style="background: linear-gradient(90deg, #2c3e50 0%, #34495e 100%); color: white;">
        <h5 class="mb-0"><i class="fas fa-edit"></i> Edit Armada</h5>
      </div>
      <div class="card-body">
        <form method="POST" action="{{ route('admin.armadas.update', $armada) }}">
          @csrf
          @method('PUT')
          <div class="mb-3">
            <label class="form-label"><i class="fas fa-bus"></i> Nama Bus</label>
            <input name="name" class="form-control form-control-lg" value="{{ $armada->name }}" required>
            @error('name')<span class="text-danger small">{{ $message }}</span>@enderror
          </div>
          <div class="mb-3">
            <label class="form-label"><i class="fas fa-chair"></i> Kapasitas Kursi</label>
            <input type="number" name="capacity" class="form-control form-control-lg" value="{{ $armada->capacity }}" min="1" required>
          ...
          <div class="mb-3">
            ...
            <input type="number" step="0.01" name="price_per_km" class="form-control form-control-lg" value="{{ $armada->price_per_km }}" min="0" required>
          ...
          <div class="mb-3">
            ...
            <select name="status" class="form-control form-control-lg">
              <option value="available" {{ $armada->status=='available' ? 'selected':'' }}>Tersedia</option>
